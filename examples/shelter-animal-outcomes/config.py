...
def transform_batch(data: pd.DataFrame, train: bool = True) -> pd.DataFrame:
    ...
    data['AgeuponOutcome'] = \
        data['AgeuponOutcome'].str.replace(r'years?', '* 365.0')
    data['AgeuponOutcome'] = \
        data['AgeuponOutcome'].str.replace(r'months?', '* 30.417')
    data['AgeuponOutcome'] = \
        data['AgeuponOutcome'].str.replace(r'weeks?', '* 7.0')
    data['AgeuponOutcome'] = \
        data['AgeuponOutcome'].str.replace(r'days?', '')
    data['AgeuponOutcome'] = \
        data['AgeuponOutcome'].apply(
